...
def main(args):
    ...
    if os.path.exists(prediction) and force is False:
        ...
        for s, seq in enumerate(test_seqs):
            ...
            std_file = '%s/%s_standardized.nii.gz' % (pred_dir, os.path.basename(seq).split('.')[0])
            nib.save(nib.Nifti1Image(std_img, t1_img.affine), std_file)

            # cropping
            if training_mods[s] == 't1':
                crop_file = '%s/%s_standardized_cropped.nii.gz' % (pred_dir, os.path.basename(seq).split('.')[0])
                trim(std_file, crop_file, voxels=1)
            else:
                crop_file = '%s/%s_standardized_cropped.nii.gz' % (pred_dir, os.path.basename(seq).split('.')[0])
                ref_file = '%s/%s_standardized_cropped.nii.gz' % (pred_dir, os.path.basename(t1).split('.')[0])
            ...
            res_file = '%s/%s_resampled.nii.gz' % (pred_dir, os.path.basename(seq).split('.')[0])
            resample(crop_file, pred_shape[0], pred_shape[1], pred_shape[2], res_file, interp=1)
